...
exports.handler = async (event) => {
  try {
    ...
    try {
      // Try to load pre-fetched series characters data
      const seriesCharactersPath = path.join(__dirname, 'series-characters.json');
      let seriesCharacters = {};
      
      if (fs.existsSync(seriesCharactersPath)) {
        seriesCharacters = JSON.parse(fs.readFileSync(seriesCharactersPath, 'utf8'));
        console.log('Loaded pre-fetched series characters data');
      }
      ...
      // Check if we have pre-fetched data for this series
      if (seriesCharacters[seriesSlug] && seriesCharacters[seriesSlug].length > 0) {
        console.log(`Using pre-fetched character data for ${seriesSlug}`);
        
        // Use the pre-fetched data
        for (const character of seriesCharacters[seriesSlug]) {
          if (character.image) {
            castMembers.push({
              name: character.name,
            ...
            console.log(`Added ${character.name} to cast list from pre-fetched data`);
          }
        }
      } else {
        console.log(`No pre-fetched character data found for ${seriesSlug}, falling back to characters.json`);
        ...
        // Define cast members for each series (fallback)
        const seriesCast = {
        ...
        // Get cast for the current series from the fallback
        ...
        for (const castMember of seriesCastMembers) {
          ...
          if (char?.wikiImage) {
            ...
            console.log(`Added ${castMember.name} to cast list (fallback)`);
          }
        }
      ...
      console.log(`Final cast list: ${castMembers.length} characters`);
    } catch (error) {
